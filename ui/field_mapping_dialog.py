...
from PySide6.QtWidgets import (
    ...
    QLineEdit, QMessageBox, QFileDialog, QGroupBox, QAbstractItemView,
...
class CandidateFieldList(QWidget):
    ...
    def _setup_ui(self):
        ...
        self.add_btn.setStyleSheet("""
            QPushButton {
                background: #4a9eff; color: white; border: 1px solid #3a8eef;
...
class ShpConfigWidget(QWidget):
    ...
    def _setup_ui(self):
        ...
        self.auto_match_btn.setFixedHeight(28)
        ...
        self.match_hint_label.setStyleSheet("color: #7f8c8d; font-size: 11px;")
        ...
        auto_add_btn.setFixedHeight(28)
    ...
    def _auto_match_output(self):
        ...
        if matched_output:
            ...
            self.match_hint_label.setStyleSheet("color: #27ae60; font-size: 11px;")
            self.custom_output_edit.setVisible(False)
            ...
            self.match_hint_label.setStyleSheet("color: #e67e22; font-size: 11px;")
    ...
    def _on_output_changed(self, output_name: str):
        ...
        if output_name == "【自定义图层】":
            ...
            self.match_hint_label.setStyleSheet("color: #3498db; font-size: 11px;")
            self._extract_keywords_from_filename()
            self._field_mapping = {}
            self._refresh_table()
        else:
...
class FieldMappingDialog(QDialog):
    ...
    def _setup_ui(self):
        ...
        # ========== 顶部：添加SHP ==========
        add_group = QGroupBox("添加SHP文件")
        add_layout = QHBoxLayout()

        self.add_shp_btn = QPushButton("选择SHP文件...")
        self.add_shp_btn.setFixedHeight(32)
        self.add_shp_btn.setCursor(Qt.PointingHandCursor)
        self.add_shp_btn.setStyleSheet("""
            QPushButton {
        ...
        add_layout.addWidget(self.add_shp_btn)
        ...
        self.config_title.setStyleSheet("font-weight: bold; font-size: 14px; color: #2c3e50;")
        ...
        self.empty_hint.setStyleSheet("color: #95a5a6; font-size: 16px;")
        ...
        self.export_rules_btn.setFixedHeight(32)
        self.export_rules_btn.setCursor(Qt.PointingHandCursor)
        self.export_rules_btn.setStyleSheet("""
            QPushButton {
                background: #17a2b8; color: white;
                border: 1px solid #138496;
                font-size: 13px; padding: 0 12px;
    ...
    def _refresh_shp_list(self):
        ...
        for row, cfg in enumerate(self._shp_configs):
            ...
            if output_name:
                ...
                output_item.setForeground(QColor('#27ae60'))
            elif keywords:
                # 自定义图层，有关键词但未设置输出名称
                output_item = QTableWidgetItem(f"[自定义] {', '.join(keywords[:2])}")
                output_item.setForeground(QColor('#e67e22'))
            else:
                output_item = QTableWidgetItem('未设置')
                output_item.setForeground(QColor('#e74c3c'))
