...
# 天地图配置文件名
TIANDITU_CONFIG_FILE = "tianditu_config.json"

# 默认天地图Key（开发测试用）
DEFAULT_TIANDITU_KEY = "fced2cd1905b5b42d4167603c6ba32e2"
...
class TiandituConfig:
    ...
    def __init__(self, config_dir: str = None):
        ...
        self.config_file = os.path.join(config_dir, TIANDITU_CONFIG_FILE)
        self._api_key = None
        ...
        if os.path.exists(self.config_file):
            try:
                with open(self.config_file, 'r', encoding='utf-8') as f:
                    data = json.load(f)
                    self._api_key = data.get('api_key', DEFAULT_TIANDITU_KEY)
                    return self._api_key
            except Exception:
                pass

        self._api_key = DEFAULT_TIANDITU_KEY
        return self._api_key
    ...
    def reset_to_default(self) -> bool:
        return self.save_api_key(DEFAULT_TIANDITU_KEY)
